...
void bit_inv(char *array, int index) {
  bit_isset(array, index) ? bit_clear(array, index) : bit_set(array, index);
}
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  xor_ab_alloc:");
    bd_print_vector(bd_sizes[k].xor_ab_alloc, NBLK(k)/2);
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_inv(bd_sizes[k].xor_ab_alloc, blk_index(k, p)/2);
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_inv(bd_sizes[k - 1].xor_ab_alloc, blk_index(k - 1, p)/2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_inv(bd_sizes[k].xor_ab_alloc, bi/2);           // free p at size k
    if (bit_isset(bd_sizes[k].xor_ab_alloc, bi/2)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      bit_inv(bd_sizes[k].xor_ab_alloc, bi/2);
    }
  }
}
...
int bd_initfree_pair(int k, int bi, int is_right) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset(bd_sizes[k].xor_ab_alloc, bi/2)) {
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    bd_sizes[k].xor_ab_alloc = p;
    memset(bd_sizes[k].xor_ab_alloc, 0, sz);
    p += sz;
  }
